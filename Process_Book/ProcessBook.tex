...
\begin{itemize}
	\item \textbf{Navigation Bar: }  The top of the page displays the actor name, a photo, and links back to the search page
	\item \textbf{Main Views: } The main views are displayed centered on the screen, with their title text centered above them
	\item \textbf{View Selector: } On the right hand side of the screen, the user can click the listed dots to jump to the next graphic.  They may also scroll, and when in the vicinity of the visualization, the browser will auto-center the visualization after a short dwell time.
\end{itemize}
...
Each of these lines support highlight-on-hover and will display a tool-tip with the movie title.

mousing over the movie list on the left will selectively increase the title font size on hover.

Each of the axes are scrubable and will highlight any films in the given selection.
\newpage 
...
The treemap visualization was a last-minute addition to our project.  Despite TA recommendation for such a view, We did not orginally plan to visualize a given actors relationship to other co-stars.  However, after the ``Visualizing Maps and Trees'' lecture, we saw a way to group co-stars which have appeared in multiple films alongside the queried actor.
	...
	\begin{figure}[h!]
		...
		\begin{subfigure}[t]{.8\textwidth}
		  ...
		  \caption{Common films between Owen and Ben Stiller}
		  \label{fig:treemapB}
		\end{subfigure}%
